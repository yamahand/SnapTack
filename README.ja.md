...
- **手元にある画像からも付箋を作れる** — トレイメニューまたは付箋上の `Ctrl+V` でクリップボードから貼り付け、あるいは画像ファイルをスクラップリストや `SnapTack.exe` へドラッグ&ドロップ
- **外部から操作できる** — コマンドラインで画像ファイルを渡せるほか、`/C:Capture`・`/C:Option`・`/R:X,Y,W,H` に対応。起動中のインスタンスへ引き渡されるので二重起動しない
...
| **ドラッグ&ドロップ** | スクラップリストウィンドウの好きな場所、または `SnapTack.exe` 自体へ画像ファイルをドロップ |
| **コマンドライン** | `SnapTack.exe "C:\pic.png"` |
...
引数で渡した画像ファイルはカーソル位置に付箋として貼られます。SnapTack が既に起動している場合、引数はそのインスタンスへ引き渡されるため **二重起動しません**。exe へのドロップも同じ経路を通り、付箋が増えるだけです。
...
`/R:` は自動化向けのオプションなので、`/C:Capture` と同時に指定した場合は `/R:` が優先されます。解釈できないオプションはエラーにせず無視します (無人実行がダイアログで止まらないようにするため)。引数なしで起動した場合の挙動は従来どおりで、常駐中なら何も起こりません。
