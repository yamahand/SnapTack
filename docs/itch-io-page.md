...
| Pricing | **`No payments`** (無料のみ。寄付は受け付けない) |

#### 寄付を受け付けない理由 (2026-07 時点の判断)

寄付を有効にするには itch.io に支払い先 (Stripe / PayPal) の接続が必要で、
未接続だと次の警告が出る:

> You don't have payment configured. If you set a minimum price above 0 no one will be able to download your project.

支払い先の接続には本人確認に加えて**米国の税務申告 (W-8BEN 相当)** が要る。
itch.io は米国企業のため、申告しないと支払いから 30% 源泉徴収される
(日米租税条約の適用を申告すれば軽減されうる)。

MIT ライセンスの OSS で、まず使ってもらう段階では割に合わないと判断し、
**無料配布のみとする**。寄付は後からいつでも追加できる。

後で寄付を有効にする場合:

1. Account → Payment settings で Stripe / PayPal を接続 (税務情報の入力を含む)
2. Pricing を **`$0 or donate`** にする。**最低価格は 0 のままにすること**
   (0 より上げると無料ダウンロードができなくなる)
3. 説明文に寄付を促す一文を戻す (無料方針に合わせて現在は削除済み)
